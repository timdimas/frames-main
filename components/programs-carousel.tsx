...
export function ProgramsCarousel() {
    ...
    return (
        <div id="programs" className="w-full flex flex-col items-center justify-center pt-24 mb-24">
            <h2 className="text-center text-3xl font-bold mb-4">Our Programs</h2>
            <Carousel
                opts={{
                    align: "center",
                }}
            ...
            >
                <CarouselContent>
                    {data?.data?.map((program: any, index: string) => (
                                ...
                                <Card>
                                    <CardContent className="flex aspect-square items-start justify-start flex-col p-0 ">
                                        ...
                                        <div className="p-2 px-3 flex flex-col gap-2">
                                            <h3 className="text-xl text-start tracking-normal">{program?.attributes?.title}</h3>
                                            <p className="text-gray-500 text-sm">12 Feb 2023</p>
                                            <p className="text-truncate text-muted-foreground line-clamp-[7]">{program?.attributes?.summary}</p>
                                        </div>
                                    </CardContent>
                                </Card>
                            </Link>
                ...
                <div className="flex justify-center mt-4 w-full">
                    {Array.from(Array(count).keys()).map((i) => (
                        <Button
                            ...
                            className={`mx-1 h-1.5 w-10 rounded-full p-0  ${i === current - 1 ? "hover:bg-primary bg-primary" : "bg-neutral-600/75"}`}
